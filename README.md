...
Чтобы проверить код:
  1) загрузите оба файла ("LimitedH_devpub_tfidf_example.ipynb" и "steam_games_cleaned.csv");
  2) поместите эти файлы в одну папку;
  3) откройте файл с кодом "LimitedH_devpub_tfidf_example.ipynb" и нажмите "Run All".
...
Примечание: основной файл с кодом "LimitedH_devpub_tfidf_example.ipynb" называется так, потому что:
  1) выборка игр в этой модели жёстко ограничена по цене ($4.99-$29.99) - LimitedH
  2) в модели используются данные о прошлых играх разработчиков - devpub
  3) тэги (отличительные особенности игр) обработаны 
